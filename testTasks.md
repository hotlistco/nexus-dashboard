Put this in `nexus-dashboard/server/src/providers/tasks.js`:
...
This provider gives the frontend a cleaner shape:

* `items`: flattened, sorted list
* `groups.overdue`
* `groups.today`
* `groups.upcoming`
* `groups.completed`
* `summary` counts for badges or headers
...
You should get JSON shaped roughly like:
...
If you want the frontend Mode E to look better, the next useful change is to have it render:

* overdue first in red
* today in white
* upcoming muted
* completed dimmed with strikethrough

I can give you that `App.jsx` patch too.
