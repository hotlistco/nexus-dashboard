...
export async function getNytHome() {
  const response = await fetch(FEED_URL);
  if (!response.ok) throw new Error(`NYT RSS fetch failed: ${response.status}`);
  const xml = await response.text();
  const items = parseItems(xml);
  if (!items.length) throw new Error('No items parsed from NYT RSS feed');
  return { items };
}
